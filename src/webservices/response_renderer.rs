    ...
    T: serde::ser::Serialize,
{
    ...
    fn respond_to(self, req: &actix_web::HttpRequest) -> actix_web::HttpResponse<Self::Body> {
        debug_span!("responsive_render_content").in_scope(|| {
            let body = if req.headers().get("HX-Request").is_some()
                ...
                let body = debug_span!("render_template", meta.hb.template = self.template)
                    .in_scope(|| self.hb.render(self.template, &self.data))
                    .unwrap();
